...
for(i in 1) {
  ui <- dashboardPage(
    dashboardHeader(title = "DEL Analysis App 2.0"),
    dashboardSidebar(
      sidebarMenu(
        id = "menu1",
        menuItem(
        ...
        menuItem(
          ...
          # menuSubItem("Add new samples", tabName = "new_samples"),
          menuSubItem("Sequencing runs", tabName = "run_meta"),
          menuSubItem("Merged table", tabName = "merged_meta")
        ),
        menuItem(
        ...
        menuItem(
          ...
          menuSubItem("3D trisynthon scatterplot", tabName = "interactive_3d_sp"),
          menuSubItem("2D scatterplot", tabName = "interactive_2d_sp")
        )
      )
    ),
    ...
    dashboardBody(
      tabItems(
        ...
        #                 DT::dataTableOutput("new_samples_table"), style = "overflow-x: scroll;",
        #                 br(), actionButton("append_samp_meta", "Add to sample metadata"))
        #           )
        #         )
        # ),
        ...
        tabItem(tabName = "merged_meta",
                fluidPage(
                  ...
                  div("Table to aid in defining analysis experiments. Click on rows to select them, then click 'Save selection' to export them as a csv file."),
                  br(),
                  fluidRow(
                    box(title = "Column selection", width = 12, status = "primary", solidHeader = T, collapsible = T, collapsed = T,
                        ...
                        actionButton("reload_merged_meta", "Reload table"))
                  ),
                  fluidRow(
                    box(title = "Merged metadata table", width = 12, status = "primary", solidHeader = T, 
                        DT::dataTableOutput("merged_meta_table"), style = "overflow-x: scroll;")
                  ),
                  fluidRow(
                         ...
                         box(title = "Scatter plot", status = "primary", solidHeader = T, width = NULL,
                             fluidPage(tags$head(tags$script('
                                       ...
                                       conditionalPanel('input.input_interactive_vis == "On"',
                                                        ...
                                                        br(),
                                                        downloadButton("output_interactive_sp_save_plot", "Save plot"),
...
server <- function(input, output, session) {
  rv <- reactiveValues(rv_lib_meta = lib_meta,
                       ...
                       rv_cpd_structure = tibble(lib_id = numeric(),
                                                 ...
                                                 cycle3 = numeric(),
                                                 structure = character()),
                       # rv_lib_enum_struct_map = NULL,
                       # rv_enum_struct_loaded = c(),
                       ...
                       rv_tbl_viewer = NULL,
                       # rv_3d_er_sp_interactive_cycle_id_choices = bb_meta %>%
                       #   select(lib_id, cyc, tag_id) %>%
                       #   mutate(value = T),
                       rv_3d_er_sp_interactive_cycle_selections = input_3d_er_sp_interactive_cycle_selections,
                       # rv_3d_er_sp_interactive_cycle1_selections = NULL,
                       # rv_3d_er_sp_interactive_cycle2_selections = NULL,
                       # rv_3d_er_sp_interactive_cycle3_selections = NULL,
  ...
  output$dashboard_screens_timeline <- renderPlot({
    temp <- rv$rv_samp_meta %>%
      left_join(rv$rv_scr_meta) %>%
      left_join(rv$rv_comb_lib_meta) %>%
  ...
  #   }
  #   else if(length(rv$rv_lib_enum_struct_map) > 0) {
  #     load_cpd_notif <- showNotification("Loading metadata...", duration = NULL)
  #     rv$rv_3d_er_sp_interactive_struct_map <- rv$rv_lib_enum_struct_map[grep(paste0(temp_lib_id, " "), names(rv$rv_lib_enum_struct_map))]
  #     removeNotification(load_cpd_notif)
  ...
  #     rv$rv_lib_enum_struct_map <- structure(temp$structure, names = paste(temp$lib_id,
  #                                                                   temp$cycle1,
  #                                                                   temp$cycle2,
  #                                                                   temp$cycle3))
  #     rv$rv_3d_er_sp_interactive_struct_map <- rv$rv_lib_enum_struct_map[grep(paste0(temp_lib_id, " "), names(rv$rv_lib_enum_struct_map))]
  ...
  #   if(length(rv$rv_lib_enum_struct_map) > 0) {
  #     temp_lib_id <- as.numeric(unlist(str_split(input$input_3d_er_interactive_lib_id, ": "))[1])
  #     load_cpd_notif <- showNotification("Loading metadata...", duration = NULL)
  #     rv$rv_3d_er_sp_interactive_struct_map <- rv$rv_lib_enum_struct_map[grep(paste0(temp_lib_id, " "), names(rv$rv_lib_enum_struct_map))]
  #     removeNotification(load_cpd_notif)
