...
class Library(object):
    ...
    def __init__(self, config):
        ...
        self.df_cpds = self.df_cpds[self.df_cpds['lib_id'].isin(library_ids)]
        self.df_tags = self.df_tags[self.df_tags['lib_id'].isin(library_ids)]
        self.df_libs = self.df_libs[self.df_libs['lib_id'].isin(library_ids)]
        ...
        for lib_id in self.tags:
            for cyc_idx in sorted(self.df_tags[self.df_tags['lib_id']==lib_id]['cyc'].unique()):
                self.tags[lib_id][cyc_idx] = {}
                # self.structures[lib_id][cyc_idx] = {}
        for i, row in self.df_tags.iterrows():
            self.tags[row['lib_id']][row['cyc']][row['cyc_seq']] = row['tag_id']
            # self.structures[row['lib_id']][row['cyc']][row['tag_id']] = row['bb_smiles']

        self.lib_tags = {}
        self.lib_structures = {}
        for i, row in self.df_libs.iterrows():
            self.lib_tags[row['lib_seq']] = row['lib_id']
            self.lib_structures[row['lib_id']] = row['lib_name']
...
class Analysis(object):
    ...
    def load(self, config):
        ...
        for exp_set in config['datasets']:
            for i,fname in enumerate(config['datasets'][exp_set]['fname']):
                self.experiments['{}_part{}'.format(config['datasets'][exp_set]['id'],i+1)] = Experiment(fname)
    ...
    def run_counts(self):
        ...
        for exp_set in self.config['datasets']:
            # Check if counts have been processed
            results_exist = True
            for lib_id in self.config['datasets'][exp_set]['library_ids']:
                subset_outfile = self.config['datasets'][exp_set]['id']+"_lib{:03}.csv".format(lib_id)
                if not os.path.isfile(subset_outfile):
                    results_exist = False
            if results_exist:
                logging.info(f'Already processed experiment {exp_set}, skipping')
                continue
            
            logging.info(f'Running analysis for experiment {exp_set}')
            
            count_list = np.zeros((len(self.results),), dtype=int)
            for i,fname in enumerate(self.config['datasets'][exp_set]['fname']):
                exp = '{}_part{}'.format(self.config['datasets'][exp_set]['id'],i+1)
                ...
                # Process counts
                logging.info(f'Processing count data for {exp}')

                # There must be a better way to do this...
                for j, barcode in tqdm(enumerate(zip(self.results['lib_id'],
                                                     self.results['cycle1'],
                                                     self.results['cycle2'],
                                                     self.results['cycle3']))):
                    if barcode in counts:
                        count_list[j] = count_list[j] + counts[barcode]
                
                logging.info('    found {}/{} tags ({:.2%})'.format((count_list>0).sum(), len(count_list), (count_list>0).mean()))

                # Save individual counts tables
                if i+1 == len(self.config['datasets'][exp_set]['fname']):
                    totals = pd.DataFrame({'value': count_list})
                    for lib_id in self.config['datasets'][exp_set]['library_ids']:
                        lib_subset = totals[self.results['lib_id'] == lib_id]
                        subset_outfile = self.config['datasets'][exp_set]['id']+"_lib{:03}.csv".format(lib_id)
                        lib_subset.to_csv(subset_outfile, index = False)
    ...
    def update_results(self, exp, counts):
        ...
        # There must be a better way to do this...
        count_list = np.zeros((len(self.results),), dtype=int)
        for i, barcode in tqdm(enumerate(zip(self.results['lib_id'], 
